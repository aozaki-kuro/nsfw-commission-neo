...
const IllustratorInfo = ({ commission, characterName }: IllustratorInfoProps) => {
  ...
  return (
    <div className="flex flex-col font-mono text-sm text-p-light md:text-xs dark:text-gray-300">
      {/* 第一行：日期和创作者/描述 */}
      <div className="flex items-center">
        <div className="flex items-center space-x-2">
          ...
          {/* 如果同时存在创作者和描述，在大屏幕上显示描述 */}
          {hasBoth && <span className="hidden md:inline"> | {description}</span>}
        </div>

        {/* 如果不同时存在创作者和描述，链接显示在第一行并靠右 */}
        {!hasBoth && <div className="ml-auto">{createLinks({ links, designLink })}</div>}
      </div>

      {/* 如果同时存在创作者和描述，链接显示在第二行并靠右 */}
      {hasBoth && <div className="mt-2 flex justify-end">{createLinks({ links, designLink })}</div>}
    </div>
  )
}
